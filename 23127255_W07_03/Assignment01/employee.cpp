...
Employee::Employee(Employee &&other) noexcept : id(std::move(other.id)), fullName(std::move(other.fullName)), hireDate(std::move(other.hireDate)), address(std::move(other.address)) {}
...
std::ostream &operator<<(std::ostream &os, const Employee &employee) {
	return os << employee.id << ';' << employee.fullName << ';' << employee.hireDate << ';' << employee.address;
}
